...
extern "C" __global__ void fill_bool(bool *buf, bool value,
                                     const size_t numel) {
  fill_with(buf, value, numel);
}
extern "C" __global__ void fill_u8(uint8_t *buf, uint8_t value,
                                   const size_t numel) {
  fill_with(buf, value, numel);
}
extern "C" __global__ void fill_u32(uint32_t *buf, uint32_t value,
                                    const size_t numel) {
  fill_with(buf, value, numel);
}
...
#ifdef HALF
#include "cuda_fp16.h"
extern "C" __global__ void fill_f16(__half *buf, __half value,
                                    const size_t numel) {
  fill_with(buf, value, numel);
}
#endif
...
#ifdef BFLOAT
#include "cuda_bf16.h"
extern "C" __global__ void fill_bf16(__nv_bfloat16 *buf, __nv_bfloat16 value,
                                     const size_t numel) {
  fill_with(buf, value, numel);
}
#endif